...
test("booking uses shared client, protects overlapping slots, schedules reminders and deduplicates replay", async () => {
  const f = await fixture();
  const booking = await f.svc.create(f.uid, f.b.public_id, f.body);
  assert.equal(booking.status, "confirmed");
  const replay = await f.svc.create(f.uid, f.b.public_id, f.body);
...
test("failed reschedule preserves old reservation; cancellation frees it and retains history", async () => {
  const f = await fixture();
  const a = await f.svc.create(f.uid, f.b.public_id, f.body);
  const b = await f.svc.create(f.uid, f.b.public_id, {
    ...f.body,
    request_key: randomUUID(),
    starts_at: f.slots.at(-1),
  });
  await assert.rejects(
    f.svc.change(f.uid, f.b.public_id, a.id, {
      action: "reschedule",
      revision: 1,
  ...
  await f.svc.change(f.uid, f.b.public_id, a.id, {
    action: "cancel",
    revision: 1,
  });
  assert.ok(
    ...
    ).includes(a.starts_at.toISOString()),
  );
  assert.equal(
    (
      await db
        .selectFrom("booking_history")
        .selectAll()
...
test("date exception overrides weekly hours and access is business scoped", async () => {
  const f = await fixture();
  await f.svc.configure(f.uid, f.b.public_id, {
    kind: "exception",
    specialist_id: f.resource.id,
    date: f.date,
    intervals: [],
    reason: "Отпуск",
  });
  ...
  await assert.rejects(
    f.svc.create(f.uid, f.b.public_id, f.body),
    (e) => e.code === "BOOKING_SLOT_UNAVAILABLE",
  );
  await assert.rejects(
    f.svc.catalog(randomUUID(), f.b.public_id),
    (e) => e.code === "BUSINESS_NOT_FOUND",
  );
});
test(
  "two simultaneous requests cannot reserve the same specialist slot",
...
test("Telegram booking menu creates linked booking and queues only current reminder once", async () => {
  const { routeBot } = await import("../src/server/bot/router.ts");
  const { queueBookingReminder } = await import(
    "../src/server/booking/worker.ts"
  );
  const f = await fixture();
  ...
  const booking = await db
    ...
    .where("business_id", "=", f.b.id)
    .executeTakeFirstOrThrow();
  assert.equal(booking.source, "telegram");
  assert.equal(
    (
      await db
        .selectFrom("telegram_dialog")
