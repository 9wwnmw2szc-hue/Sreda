...
INSERT INTO client(id,business_id,name,first_seen_at,last_seen_at,created_at,updated_at)
 SELECT m.client_id,m.business_id,coalesce(nullif(left(btrim(c.external_username),100),''),'Клиент'),c.created_at,c.last_message_at,c.created_at,c.last_message_at
...
INSERT INTO client(id,business_id,name,phone,first_seen_at,last_seen_at,created_at,updated_at)
 SELECT m.client_id,m.business_id,coalesce(nullif(left(btrim(l.name),100),''),'Клиент'),l.phone,l.created_at,l.updated_at,l.created_at,l.updated_at FROM crm_lead_backfill m JOIN lead l ON l.id=m.source_id;
