...
function ConnectionSetup({ currentBusiness }: { currentBusiness: Business }) {
  ...
  return <div className="connections-page"><header><span className="eyebrow">Каналы бизнеса</span><h1>Подключения</h1><p>Подключите Telegram и VK к единому ядру «Среды».</p></header>
    <p className="account-footnote">Пока доступна проверка токена. Приём сообщений и вход по коду ещё не запущены.</p><section className="panel connection-setup"><div className="connection-setup__title"><Bot size={24} /><div><h2>Telegram</h2><p>Токен бота от @BotFather</p></div>{telegram?.status === "connected" && <CheckCircle2 className="connection-ok" size={22} />}</div>
      {telegram?.status === "connected" ? <><p className="account-notice">Токен проверен: {telegram.displayName ?? "Telegram-бот"}</p><button className="button button--outline" type="button" disabled={busy} onClick={() => void disconnectTelegram()}>Отключить Telegram</button></> : <><label htmlFor="telegram-token">Токен бота</label><input id="telegram-token" type="password" autoComplete="off" value={token} onChange={(e) => setToken(e.target.value)} placeholder="123456:AA..." disabled={busy} /><p className="account-footnote"><ShieldCheck size={15} />Токен передаётся по защищённому соединению и не показывается после сохранения.</p><button className="button button--primary" type="button" disabled={busy || !token.trim()} onClick={() => void connectTelegram()}>{busy && <LoaderCircle className="spin" size={17} />}{busy ? "Проверяем…" : "Подключить Telegram"}</button></>}
