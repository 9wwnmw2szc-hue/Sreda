...
export function MembersPanel({ business }: { business?: Business }) {
  ...
  async function accept(id: string) { setBusy(true); try { await apiRequest(`/api/v1/invitations/${id}/accept`, { method: "POST", body: "{}" }); setIncoming((items) => items.filter((item) => item.id !== id)); router.push("/dashboard"); router.refresh(); } catch (e) { setError(e instanceof Error ? e.message : "Не удалось принять приглашение."); } finally { setBusy(false); } }
