...
export function NotificationSettings({ businessId }: { businessId: string }) {
  ...
  async function save(body: object) {
    ...
    setError("");
    try {
      const result = await apiRequest<{ command?: string }>(url, {
        method: "POST",
  ...
  return (
    <section className="panel crm-panel">
      ...
      {!data ? (
        ...
        <>
          <p>
            Telegram для вашего аккаунта:{" "}
            {data.connected ? "подключён" : "не подключён"}.
          </p>
          <button
            disabled={busy}
            onClick={() => void save({ action: "connect" })}
          >
            Получить код подключения
          </button>
          {data.connected && (
            <button disabled={busy} onClick={() => setConfirm(true)}>
              Отключить мои уведомления в Telegram
            </button>
          )}
          {confirm && (
            <div role="dialog" aria-label="Отключить уведомления">
              <p>Отключить Telegram-уведомления для вашего аккаунта?</p>
              <button
                disabled={busy}
                onClick={() => void save({ action: "disconnect" })}
              >
                Отключить
              </button>
              <button onClick={() => setConfirm(false)}>Назад</button>
            </div>
          )}
          {command && (
            <div role="status">
