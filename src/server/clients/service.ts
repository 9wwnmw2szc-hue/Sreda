...
export class ClientService {
 ...
 async list(userId:string,publicId:string,search='',before?:string,filter='all'){
  ...
  if(before){if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(before))throw new AppError(400,'INVALID_CURSOR','Обновите список.');q=q.where('c.id','>',before);}
 ...
 async detail(userId:string,publicId:string,id:string){
  const b=await requireBusiness(this.db,userId,publicId,'clients.read');
  if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id))throw new AppError(404,'CLIENT_NOT_FOUND','Клиент не найден.');
