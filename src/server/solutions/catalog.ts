export type SolutionCode = "leads" | "sales" | "autopost" | "booking" | "admin_messages" | "moderation";
...
export const SOLUTIONS: readonly SolutionDefinition[] = [
  ...
  { code: "sales", id: "sol_sales", title: "Продажи", description: "Товары, заказы и клиенты в одном боте.", priceRub: 250, menu: ["Товары", "Заказы", "Клиенты"] },
...
export function solutionByCode(code: string) {
  return SOLUTIONS.find((solution) => solution.code === code);
}

export function solutionMenu(activeCodes: readonly string[]) {
  return SOLUTIONS.filter((solution) => activeCodes.includes(solution.code)).flatMap((solution) => solution.menu);
}
