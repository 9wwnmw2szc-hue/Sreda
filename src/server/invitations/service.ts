...
export class InvitationService {
  ...
  private async mutate<T>(
    ...
    action: (service: InvitationService) => Promise<T>,
  ): Promise<T> {
    return this.db.transaction().execute(async (tx) => {
      ...
      await tx
        .selectFrom("business")
        .select("id")
        .where("public_id", "=", businessPublicId)
        .forUpdate()
        .execute();
  ...
  private async audit(
    ...
    businessId: string,
    actorUserId: string,
    action: Database["business_audit_log"]["action"],
  ...
  private async business(
    ...
    const row = await this.db
      ...
      .where("business.public_id", "=", businessPublicId)
  ...
  async create(
    inviterId: string,
    businessPublicId: string,
    targetPublicId: unknown,
    role: unknown,
  ): Promise<ReturnType<InvitationService["output"]>> {
    if (!this.transactional)
      return this.mutate(businessPublicId, (service) =>
        service.create(inviterId, businessPublicId, targetPublicId, role),
      );
    const businessId = await this.business(inviterId, businessPublicId);
    if (
      typeof targetPublicId !== "string" ||
      !publicId.test(targetPublicId) ||
      !["admin", "operator"].includes(String(role))
    )
      throw new AppError(
        400,
        "INVALID_INVITATION",
        "Проверьте ID пользователя и роль.",
      );
    ...
    if (member)
      throw new AppError(
        409,
        "MEMBER_EXISTS",
        "Пользователь уже участвует в бизнесе.",
      );
    await this.db
    ...
    if (existing)
      throw new AppError(
        409,
        "INVITATION_EXISTS",
        "Для этого пользователя уже есть приглашение.",
      );
    const row = await this.db
      .insertInto("business_invitation")
      .values({
        id: randomUUID(),
        business_id: businessId,
  ...
  async list(userId: string, businessPublicId?: string) {
    const businessId = businessPublicId
      ? await this.business(userId, businessPublicId)
      : undefined;
    let query = this.db
      .selectFrom("business_invitation as invitation")
      .innerJoin("business", "business.id", "invitation.business_id")
      .select([
        "invitation.id",
        "invitation.role",
        "invitation.status",
        ...
        "business.public_id as businessId",
        "business.name as businessName",
      ])
      .where("business.archived_at", "is", null)
      .where("invitation.status", "=", "pending")
      .where("invitation.expires_at", ">", new Date())
      .orderBy("invitation.created_at", "desc");
    query = businessId
      ? query.where("invitation.business_id", "=", businessId)
      : query.where("invitation.invitee_user_id", "=", userId);
  ...
  async members(userId: string, businessPublicId: string) {
    ...
    return this.db
      ...
      .select([
        "user.public_id as userId",
        "user.username",
        "user.name",
        "member.role",
        "member.created_at as joinedAt",
      ])
      .where("member.business_id", "=", businessId)
      .where("member.status", "=", "active")
  ...
  async revokeMember(
    ...
    if (!this.transactional)
      return this.mutate(businessPublicId, (service) =>
        service.revokeMember(ownerId, businessPublicId, targetPublicId),
      );
    const businessId = await this.business(ownerId, businessPublicId);
    if (typeof targetPublicId !== "string" || !publicId.test(targetPublicId))
      throw new AppError(400, "INVALID_MEMBER", "Проверьте ID пользователя.");
    const target = await this.db
      .selectFrom("user")
      .select("id")
      .where("public_id", "=", targetPublicId)
      .executeTakeFirst();
    if (!target)
      throw new AppError(404, "MEMBER_NOT_FOUND", "Участник не найден.");
    ...
    await this.db
      ...
      .set({
        ...
        status: sql`case when status='assigned' then 'open' else status end`,
      })
      .where("business_id", "=", businessId)
      .where("assigned_member_user_id", "=", target.id)
      .execute();
    await this.db
      .updateTable("lead")
      .set({
  ...
  async changeRole(
    ...
    if (!this.transactional)
      return this.mutate(businessPublicId, (service) =>
        service.changeRole(ownerId, businessPublicId, targetPublicId, role),
      );
    const businessId = await this.business(ownerId, businessPublicId);
    if (
      typeof targetPublicId !== "string" ||
    ...
    const changed = await this.db
      .updateTable("business_member")
      .set({ role: role as "admin" | "operator" })
      .where("business_id", "=", businessId)
      .where("user_id", "=", target.id)
      .where("role", "!=", "owner")
      .where("status", "=", "active")
  ...
  async accept(userId: string, invitationId: string) {
    ...
    return this.db.transaction().execute(async (tx) => {
      ...
      await new InvitationService(tx, true).business(
      ...
      await tx
        ...
        .onConflict((oc) =>
          oc
            ...
            .doUpdateSet({ role: invitation.role, status: "active" }),
        )
        .execute();
      await tx
        .updateTable("business_invitation")
  ...
  async revoke(
    ...
    if (!this.transactional)
      return this.mutate(businessPublicId, (service) =>
        service.revoke(ownerId, businessPublicId, invitationId),
      );
    if (
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(
        invitationId,
  ...
  async auditLog(userId: string, businessPublicId: string) {
    const businessId = await this.business(
      userId,
      businessPublicId,
      "admin",
    ).catch(async (error) => {
      if (error instanceof AppError && error.status === 403)
        return this.business(userId, businessPublicId, "owner");
      throw error;
    });
    return this.db
      .selectFrom("business_audit_log as log")
      .innerJoin("user as actor", "actor.id", "log.actor_user_id")
      .leftJoin("user as target", "target.id", "log.target_user_id")
      .select([
        "log.id",
        "log.action",
        "log.details",
        "log.created_at as createdAt",
        "target.public_id as targetUserId",
        "actor.public_id as actorUserId",
  ...
  private output(
    row: Selectable<Database["business_invitation"]>,
    businessId: string,
  ) {
    return {
      id: row.id,
