...
export type NotificationType='lead.created'|'message.received'|'booking.created'|'booking.cancelled'|'booking.rescheduled'|'post.failed';
...
export class NotificationService{
 ...
 async list(userId:string,publicId:string){const b=await requireBusiness(this.db,userId,publicId,'notifications.read');return this.db.selectFrom('notification as n').innerJoin('notification_recipient as r','r.notification_id','n.id').select(['n.id','n.type','n.title','n.target_path','n.created_at','r.read_at']).where('n.business_id','=',b.id).where('r.user_id','=',userId).orderBy('n.created_at','desc').limit(100).execute();}
 async read(userId:string,publicId:string,id:string){const b=await requireBusiness(this.db,userId,publicId,'notifications.read');await this.db.updateTable('notification_recipient').set({read_at:new Date()}).where('business_id','=',b.id).where('user_id','=',userId).where('notification_id','=',id).execute();return {ok:true};}
}
