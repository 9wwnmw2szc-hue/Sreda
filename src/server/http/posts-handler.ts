...
export function postsHandler(
  ...
  return respond(request, async () => {
    ...
    if (request.method !== "GET") {
      ...
      await limit(r.db, r.secret, "posts:" + publicId + ":" + user.id, 30, 60);
    }
    if (resource === "post-targets")
      return json(
        request.method === "GET"
          ? await service.targets(user.id, publicId)
          : await service.connectTarget(
              user.id,
              publicId,
    ...
    if (request.method === "POST" || request.method === "PATCH")
      return json(await service.save(user.id, publicId, body, id));
