...
export class WorkspaceService {
  ...
  private query(userId: string) {
    return this.db
      .selectFrom("business")
      .innerJoin(
        "business_member as member",
        "member.business_id",
        "business.id",
      )
      .innerJoin("business_member as owner", (join) =>
        join
          .onRef("owner.business_id", "=", "business.id")
          .on("owner.role", "=", "owner")
          .on("owner.status", "=", "active"),
      )
      ...
      .select([
        "business.public_id as id",
        "business.name",
        "business.timezone",
        "member.role",
        "owner_user.public_id as ownerId",
      ]);
  }

  async list(userId: string) {
    return this.query(await this.resolveUserId(userId))
      .orderBy("business.created_at")
      .orderBy("business.id")
      .execute();
  }
  ...
  async require(userId: string, businessId: string, roles?: Role[]) {
    ...
    const business = /^biz_[a-f0-9]{20}$/.test(businessId)
      ? await this.query(userId)
          .where("business.public_id", "=", businessId)
          .executeTakeFirst()
      : undefined;
  ...
  async create(
    ...
    const businessId = await this.db.transaction().execute(async (tx) => {
      ...
      await sql`select pg_advisory_xact_lock(hashtextextended(${"business:" + userId}, 0))`.execute(
        tx,
      );
      const previous = await tx
        .selectFrom("business_creation")
        .selectAll()
      ...
      if (owned.length >= 20)
        throw new AppError(
          409,
          "BUSINESS_LIMIT",
          "Достигнут лимит: 20 бизнесов на аккаунт.",
        );
      const id = randomUUID();
      await tx
        .insertInto("business")
