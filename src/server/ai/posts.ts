...
const instructions = {
  ...
  expand: "Расширь текст без выдуманных фактов.",
  selling: "Сделай текст более продающим без недостоверных обещаний.",
  formal: "Сделай текст более официальным.",
  friendly: "Сделай текст дружелюбнее.",
  correct: "Исправь ошибки.",
  add_emoji: "Добавь уместные эмодзи.",
...
export async function generatePost(
  ...
  if (!token || !model)
    throw new AppError(
      503,
      "AI_NOT_CONFIGURED",
      "Генерация текста пока не настроена.",
    );
  ...
  try {
    ...
    if (!response.ok || data.status !== "completed") throw Error();
    const output = (data.output ?? [])
      ...
      .join("\n")
      .trim();
    if (!output || output.length > 4096) throw Error();
    console.info(
      JSON.stringify({
        code: "AI_DRAFT_CREATED",
        requestId,
