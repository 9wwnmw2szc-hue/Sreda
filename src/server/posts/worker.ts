...
import { nextOccurrence, type Recurrence } from "./recurrence.ts";
import { postDeliveryResult } from "./delivery.ts";
export async function queueScheduledPost(db: Kysely<Database>) {
  const candidate = await db
    .selectFrom("post as p")
    .innerJoin("business_solution as s", "s.business_id", "p.business_id")
  ...
  return db.transaction().execute(async (tx) => {
    ...
    const deliveries = await tx
      ...
      .innerJoin("business_connection as c", "c.id", "t.connection_id")
      .select([
        "d.id",
        "t.connection_id",
        "t.external_id",
      ...
      .where("d.post_id", "=", post.id)
      .where("d.status", "=", "pending")
      .execute();
    for (const d of deliveries) {
      if (!d.active || d.connection_status !== "connected") {
      ...
      const buttons = post.buttons as { text: string; url: string }[];
        ...
        for (const [post_step, step] of steps.entries())
          await tx
            ...
            .values({
              ...
              post_step,
              message: step.text,
              attachment_ids: JSON.stringify(step.ids),
              chat_id: d.external_id,
              api_payload: JSON.stringify(
                step.buttons.length
                  ? {
                      reply_markup: {
                        inline_keyboard: step.buttons.map((b) => [
            ...
            .onConflict((oc) =>
              oc.columns(["post_delivery_id", "post_step"]).doNothing(),
            )
            .execute();
      } else
        await tx
          ...
          .values({
            ...
            peer_id: d.external_id,
            api_payload: JSON.stringify({
              owner_id: d.external_id,
              from_group: 1,
              guid: d.id,
              message:
                post.text +
...
export async function materializeRecurringPost(db: Kysely<Database>) {
  ...
  return db.transaction().execute(async (tx) => {
    ...
    const schedule = await tx
      ...
      .where("active", "=", true)
      .where("next_at", "<=", new Date())
      .forUpdate()
      .executeTakeFirst();
    if (!schedule?.next_at) return false;
    ...
    const post = await tx
      ...
      .values({
        ...
        request_key:
          ...
          schedule.post_id +
          ":" +
          schedule.next_at.toISOString(),
        request_hash: template.request_hash,
        template_id: template.id,
        occurrence_at: schedule.next_at,
      })
      .onConflict((oc) =>
        oc.columns(["template_id", "occurrence_at"]).doNothing(),
      )
      .returning("id")
      .executeTakeFirst();
    if (post) {
    ...
    const next = nextOccurrence(schedule.rule as Recurrence, new Date());
    await tx
      .updateTable("post_schedule")
      .set({ next_at: next, active: !!next })
