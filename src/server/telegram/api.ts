...
export async function telegramCall(token:string,method:"sendMessage"|"setWebhook"|"getMe"|"getChat"|"getChatMember",body:Record<string,unknown>,transport:typeof fetch=fetch) {
 try {
  ...
  const data=await response.json().catch(()=>null) as {ok?:boolean;result?:{message_id?:number;id?:number;type?:string;title?:string;status?:string;can_post_messages?:boolean};parameters?:{retry_after?:number}}|null;
