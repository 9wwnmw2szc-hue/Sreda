...
export class TelegramError extends AppError {
  constructor(
    ...
    public chatUnavailable = false,
    public uncertain = false,
  ) {
    super(
      503,
      "TELEGRAM_UNAVAILABLE",
      "Telegram не подтвердил действие. Попробуйте позже.",
    );
  }
}
  ...
  try {
    ...
    if (!response.ok || !data?.ok)
      throw new TelegramError(
        Math.min(3600, Math.max(0, Number(data?.parameters?.retry_after) || 0)),
        [400, 401, 403, 404].includes(response.status),
        method === "sendMessage" && response.status === 403,
      );
    if (
      method === "sendMessage" &&
      (!Number.isSafeInteger(data.result?.message_id) ||
        Number(data.result?.message_id) <= 0)
    )
