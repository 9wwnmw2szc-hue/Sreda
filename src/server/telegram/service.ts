...
export class TelegramService {
 ...
 async deliverOne(){
  ...
  const candidate=await this.db.selectFrom("telegram_outbox as o").innerJoin("business_connection as c","c.id","o.connection_id").innerJoin("telegram_runtime as r","r.connection_id","c.id").innerJoin("connection_secret as credential","credential.connection_id","c.id").select(["o.id","c.business_id"]).where("c.status","=","connected").where("o.delivery_state","=","pending").where("o.delivered_at","is",null).where("o.attempts","<",8).where("o.available_at","<=",new Date()).where("r.status","=","ready").where(sql<boolean>`not exists(select 1 from telegram_outbox previous where previous.connection_id=o.connection_id and previous.chat_id=o.chat_id and previous.id<o.id and previous.delivered_at is null and previous.delivery_state in ('pending','sending'))`).where(sql<boolean>`(o.post_delivery_id is null or exists(select 1 from post_delivery d join business_solution s on s.business_id=d.business_id where d.id=o.post_delivery_id and d.status='publishing' and s.solution_code='autopost' and s.status in ('active','trial')))` ).orderBy("o.id").executeTakeFirst();
