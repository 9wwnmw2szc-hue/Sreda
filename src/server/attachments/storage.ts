...
export class FileAttachmentStorage implements AttachmentStorage {
  constructor(private root: string) {
    if (!isAbsolute(root)) throw new Error("Storage path must be absolute");
  }
  async put(key: string, bytes: Uint8Array) {
    keyCheck(key);
    await mkdir(join(this.root, key.split("/")[0]!), {
      recursive: true,
      mode: 0o700,
    });
...
export class S3AttachmentStorage implements AttachmentStorage {
  ...
  async get(key: string) {
    ...
    if (!result.Body || (result.ContentLength ?? 0) > MAX_ATTACHMENT)
