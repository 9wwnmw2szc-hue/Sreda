...
for (const size of sizes) {
  ...
  await page.screenshot({ path: `${out}/stage26_dashboard_${size.name}.png`, fullPage: false });
  if (size.name === '1440') {
    await page.screenshot({ path: `${out}/stage26_dashboard_1440_full.png`, fullPage: true });
  }
  await page.close();
